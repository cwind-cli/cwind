...
// FetchRangeContext fetches archive data and respects cancellation and deadlines.
...
// FetchRangeContextWithTimezone is the configurable timezone variant.
...
// FetchRangeContextWithCache uses a local, opt-in cache. A non-positive TTL disables it.
// Cache files contain only the provider response and are written with private permissions.
...
// FetchRangeWithClient is useful for controlled clients and httptest servers.
